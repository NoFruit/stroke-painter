...
from ..geom import AABBGeom, CapsGeom, LineGeom, StampGeom
...
class LineSquareBrush(BrushBase):
    ...
    def forward(self, params: Tensor, patch_size) -> Tuple[Tensor, Tensor]:
        ...
        新风格（geom 包）：直线几何走 LineGeom（长度/方向/弧长采样），stamp 参数走
        StampGeom，端点切线走 CapsGeom（直线两端切线相同 u0=u2=direction）；绘制侧
        （render_coverage / cut_endcaps）不变。
        ...
        # ---- 几何（r 无关）：LineGeom 缓存长度 / 方向，弧长采样 ----
        lg = LineGeom(P0p, P2p)
        # ---- stamp 参数（几何无关）：L + r_px -> s_targets / weights ----
        s_targets, weights = StampGeom.setup(
            lg.length, r_px, self.stamp_spacing_rho, self.max_stamp_K, self.sigmoid_alpha
        )
        centers = lg.sample(s_targets)                          # (B, K, 2)
        ...
        # ---- 端点切线 -> CapsGeom（r 无关，仅 square；直线 u0=u2=direction）----
        u = lg.direction                                       # (B, 2) 单位方向
        caps = CapsGeom(u, u)
    ...
    def forward_fast(self, params: Tensor, patch_size):
        ...
        段1 用 geom 包产出几何：LineGeom（长度/方向/曲线 AABB/弧长采样）、
        StampGeom（采样集 + 半径）、AABBGeom（曲线 AABB + r -> tube/square/padded/
        integer 整数框）、CapsGeom（端点切线 u0=u2=direction）。段2 由
